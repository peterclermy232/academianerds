...
<div
  ...
  className="relative"
>
  <button className="hover:text-blue-600 mr-9">
    Blogs <FontAwesomeIcon icon={faChevronDown} className="ml-1" />
  </button>
  {isBlogsOpen && (
    <ul className="absolute top-full left-0 mt-2 w-48 bg-white shadow-lg rounded-md" onMouseEnter={() => setIsBlogsOpen(true)} onMouseLeave={() => handleClose(setIsBlogsOpen)}>
      <li className="relative group">
        ...
        {/* Submenu */}
        <ul className="absolute top-0 left-full mt-0 ml-2 hidden group-hover:block w-48 bg-white shadow-lg rounded-md">
          <li>
            <a
              href="/gallery"
